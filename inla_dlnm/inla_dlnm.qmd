...
form = formula(y ~ 1 +
                 ...
                 cb_spi3 +
                 cb_precip) # unname because otherwise the names in these two matrices conflict.
```
...
plot_lag_surface = function(model, pattern) {}
...
predt <- crosspred(cb_precip, coef = coef[indt], vcov=vcov[indt,indt],
...
pdf("contour2.pdf", width = 6.5, height = 6)

y <- predt$predvar
z <- t(predt$matRRfit)
x <- seq(0, 180, length.out = dim(z)[1])
...
filled.contour(x, y, z,
    xlab = "Lag", ylab = "Precipitation (mm)", main = "",
    col = cols, levels = levels)

filled.contour(y, x, t(z),
    ylab = "Lag", xlab = "Precipitation (mm)", main = "",
    col = cols, levels = levels)
